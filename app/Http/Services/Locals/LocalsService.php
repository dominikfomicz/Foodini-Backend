...
class LocalsService
{
    ...
    public function getOrderedList($id_city_const_type, $id_sort_const_type){
        ...
        switch ($id_sort_const_type) {
            case 1:
                $locals = collect(collect(LocalsRepository::getOrderedList($id_city_const_type))->sortByDesc('favourite_count'))->all();
                foreach($locals AS $local){
                    $local->tags = collect(LocalsRepository::getTagsByLocal($local->local_id))->where('is_main', true);
                }
            break;
            ...
            case 2:
                $locals = collect(LocalsRepository::getOrderedList($id_city_const_type))->sortByDesc('create_date')->all();
                foreach($locals AS $local){
                    $local->tags = collect(LocalsRepository::getTagsByLocal($local->local_id))->where('is_main', true);
                }
            break;
